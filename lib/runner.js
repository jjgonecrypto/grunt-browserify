  ...
  run: function (files, destination, options, next) {
    ...
    if (options.watch) {
      ...
      b.on('update', function (ids) {
        ...
        doBundle.call(self, b, options, bundleUpdate);
      });
    }
    ...
    doBundle.call(self, b, options, bundleComplete);
  },
...
function doBundle(browserifyInstance, opts, bundleComplete) {
  ...
  var whenFactorBundlesReady = {
    bundles: [],
    cbs: [],
    then: function (cb) {
      this.cbs = this.cbs.concat(cb);
      if (!this.bundles.length) {
        this.resolve();
      }
    },
    resolve: function () {
      browserifyInstance.removeListener('factor.pipeline', factorBundlesReadyListener);
    ...
    onFactorPipeline: function(file, pipeline, output) {
      if (!output) {
        return;
      }
      this.bundles.push(output);
      output.once('finish', function () {
        self.logger.log.ok('Factor-Bundle ' + path.relative(process.cwd(), output.path).cyan + ' created. ');
        this.pop();
      }.bind(this));
    }
  };

  // track listener for later removal
  var factorBundlesReadyListener = whenFactorBundlesReady.onFactorPipeline.bind(whenFactorBundlesReady);

  browserifyInstance.on('factor.pipeline', factorBundlesReadyListener);

  browserifyInstance.bundle(function (err, buf) {

    if (opts.postBundleCB) {
      opts.postBundleCB(err, buf, function () {
        var args = [].slice.call(arguments);
        whenFactorBundlesReady.then(function () { bundleComplete.apply(null, args); });
      });
    } else {
      whenFactorBundlesReady.then(function () { bundleComplete(err, buf); });
      if (err) {
        whenFactorBundlesReady.resolve(); // resolve manualy as output files won't be written
      }
    }
  });
}
